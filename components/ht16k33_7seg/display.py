...
ht16k33_7seg_ns = cg.esphome_ns.namespace('ht16k33_7seg')
HT16K337SegDisplay = ht16k33_7seg_ns.class_('HT16K337SegDisplay', cg.PollingComponent, i2c.I2CDevice)
...
CONFIG_SCHEMA = display.BASIC_DISPLAY_SCHEMA.extend({
    cv.GenerateID(): cv.declare_id(HT16K337SegDisplay),
...
async def to_code(config):
    ...
    if CONF_LAMBDA in config:
        lambda_ = await cg.process_lambda(config[CONF_LAMBDA],
                                          [(HT16K337SegDisplay.operator('ref'), 'it')],
                                          return_type=cg.void)

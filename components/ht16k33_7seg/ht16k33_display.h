...
class HT16K337SegDisplay : public PollingComponent, public i2c::I2CDevice {
 public:
  void set_writer(std::function<void(HT16K337SegDisplay &)> &&writer) { this->writer_ = std::move(writer); }
  void setup() override;
  void loop() override;
 ...
 protected:
  ...
  std::function<void(HT16K337SegDisplay &)> writer_;
  bool scroll_ {false};
